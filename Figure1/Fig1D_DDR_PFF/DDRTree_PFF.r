################################################################################
# Fig.1D: DDRTree Dimensionality Reduction — PFF/fHP Endotype Identification
#
# Description:
#   Applies DDRTree (Discriminative Dimensionality Reduction via Learning a Tree)
#   to voom-normalized RNA-seq data to visualize trajectory structure and
#   separate Major/Minor endotypes in PFF and fHP/IPF training cohorts.
#
# Input files:
#   - pff.tpm.voom.fhp.ipf.csv         : Voom-normalized TPM expression matrix
#                                         (genes x samples)
#   - latend.coord.C2.allGenes.csv     : Sample metadata with endotype labels
#                                         and latent coordinates
#
# Output files:
#   - ddrtree.pff.fhp.csv              : DDRTree 2D coordinates merged with
#                                         sample metadata
...
# Sample metadata with endotype and latent coordinate information
df2 <- read.csv(file.path(data_dir, "latend.coord.C2.allGenes.csv"), row.names = 1)


# --- 2. Align Samples ---------------------------------------------------------

# Retain only samples present in both the expression matrix and metadata
...
Sex      <- meta$Sex
Cluster  <- meta$class.pred
Dx       <- meta$Cluster.Bal30      # Diagnostic cluster label
...
# Attach row/col names to coordinate matrix
