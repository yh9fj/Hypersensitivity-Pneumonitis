################################################################################
# Figure 1C: DDRTree Dimensionality Reduction — NJH Validation Cohort
#
# Description:
#   Applies DDRTree to voom-normalized RNA-seq data from the NJH validation
#   cohort to visualize trajectory structure and separate Major/Minor endotypes.
#   This script mirrors the analysis in DDRTree.R (PFF/fHP training cohort)
#   and DDRTree_UCD.R (UCD validation cohort).
#
# Input files:
#   - njh.fhp.tpm.voom.csv   : Voom-normalized TPM expression matrix
#                               (genes x samples)
#   - meta.njh.csv           : Sample metadata with endotype labels
#
# Output files:
#   - ddrtree.njh.fhp.csv    : DDRTree 2D coordinates merged with sample metadata
...
data_dir <- "../../Data/NJH.RNAseq"


# --- 1. Load Data -------------------------------------------------------------

# Expression matrix (genes x samples)
df1 <- read.csv(file.path(data_dir, "njh.fhp.tpm.voom.csv"), row.names = 1)

# Sample metadata
df2 <- read.csv(file.path(data_dir, "meta.njh.csv"), row.names = 1)
