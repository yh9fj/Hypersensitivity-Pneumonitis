################################################################################
# Figure 1I: Unsupervised PCA — NJH Validation Cohort
#
# Description:
#   Performs unsupervised PCA on the top 2,000 most variable genes from
#   voom-normalized RNA-seq data (NJH cohort). Confidence ellipses are plotted
#   per endotype (Major / Minor). PC1 separation between endotypes is assessed
#   by Welch two-sample t-test (reported p = 1.489e-05).
#   Mirrors the analysis in pca_unsupervised.R (PFF/fHP training cohort) and
#   pca_unsupervised_UCD.R (UCD validation cohort).
#
# Input files:
#   - njh.fhp.tpm.voom.csv   : Voom-normalized expression matrix (genes x samples)
#   - meta.njh.csv           : Sample metadata including endotype labels
#
# Output:
